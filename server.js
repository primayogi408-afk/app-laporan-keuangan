...
function loadPeriods() {
  try {
    ...
    for (const f of files) {
      ...
      periods.push({ key, label: label || key });
    }
...
function savePeriods(list) {
  try { fs.writeFileSync(PERIODS_PATH, JSON.stringify(list, null, 2)); } catch {}
}

function excelExists(period) {
  if (!period) return fs.existsSync(path.join(DATA_DIR, 'master.xlsx'));
  const candidates = [period, period.replace(/\s+/g,'-'), period.replace(/-/g,' '), period.toUpperCase(), period.replace(/\s+/g,'-').toUpperCase()];
  for (const c of candidates) if (fs.existsSync(path.join(DATA_DIR, `${c}.xlsx`))) return true;
  return false;
}
...
app.get('/api/periods', (req, res) => { try { res.json(loadPeriods()); } catch(e){ res.json([]); } });
...
app.delete('/api/periods/:key', (req, res) => {
  try {
    ...
    const keysToTry = [rawKey, rawKey.replace(/\s+/g,'-'), rawKey.replace(/-/g,' '), rawKey.toUpperCase(), rawKey.replace(/\s+/g,'-').toUpperCase()];
    let deletedFiles = [];
    for (const k of keysToTry) {
      const paths = [path.join(DATA_DIR, `${k}.xlsx`), editsPath(k), configPath(k), path.join(DATA_DIR, `edits_${k}.json`), path.join(DATA_DIR, `config_${k}.json`)];
      for (const p of paths) { if (fs.existsSync(p)) { try{fs.unlinkSync(p); deletedFiles.push(path.basename(p));}catch{}} }
    }
    // also try normalized
    const norm = rawKey.replace(/\s+/g,'-').toUpperCase();
    for (const ext of ['xlsx']) {
      const p = path.join(DATA_DIR, `${norm}.${ext}`);
      if (fs.existsSync(p) && !deletedFiles.includes(path.basename(p))) { try{fs.unlinkSync(p); deletedFiles.push(path.basename(p));}catch{}}
    }
    let periods = loadPeriods();
    periods = periods.filter(p => !keysToTry.includes(p.key) && !keysToTry.includes(p.key.replace(/\s+/g,'-')) && p.key.toUpperCase() !== norm);
    savePeriods(periods);
    res.json({ message: 'Deleted', deletedFiles, periods });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
