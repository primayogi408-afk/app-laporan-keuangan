...
// INIT: copy bundled Excel from repo (root or data folder) to /tmp so data persists
(function initBundledData(){
  try {
    ...
    for (const src of possibleSources) {
      if (fs.existsSync(src)) {
        ...
        if (!fs.existsSync(dest) || fs.statSync(src).size !== fs.statSync(dest).size) {
          ...
          console.log('Copied bundled', src, 'to', dest);
        }
        break;
      }
    }
    // copy other xlsx in data folder
    ...
    // ensure periods.json exists
    const perSrc = path.join(__dirname, 'periods.json');
    const perDest = path.join(DATA_DIR, 'periods.json');
    if (fs.existsSync(perSrc) && !fs.existsSync(perDest)) {
      fs.copyFileSync(perSrc, perDest);
    }
    if (!fs.existsSync(perDest)) {
      const hasMaster = fs.existsSync(path.join(DATA_DIR, 'master.xlsx'));
      if (hasMaster) fs.writeFileSync(perDest, JSON.stringify([{key:'master', label:'Utama'}], null, 2));
    }
  } catch(e){ console.log('init bundler error', e.message, e.stack); }
})();
...
// Serve from both public and root (karena upload berantakan)
...
app.get('/', (req, res) => {
  ...
  // kalau gak ada index.html, list apa yang ada biar debug
  const files = fs.readdirSync(__dirname);
  const pub = fs.existsSync(path.join(__dirname,'public')) ? fs.readdirSync(path.join(__dirname,'public')) : [];
  return res.send(`<h1>Debug: index.html tidak ditemukan</h1><p>Root files: ${files.join(', ')}</p><p>Public files: ${pub.join(', ')}</p><p>Cek GitHub, pastikan ada folder public yang isinya index.html</p>`);
});
...
function loadPeriods() {
  try {
    ...
    const periods=[];
    let savedMap={};
    try{ if(fs.existsSync(PERIODS_PATH)){ const saved=JSON.parse(fs.readFileSync(PERIODS_PATH,'utf8')); saved.forEach(p=>savedMap[p.key]=p.label); } }catch{}
    for (const f of files){
      const key = f.slice(0,-5);
      if (key.startsWith('~')) continue;
      const label = savedMap[key] || savedMap[key.replace(/\s+/g,'-').toUpperCase()] || key;
      periods.push({key, label});
    }
    periods.sort((a,b)=> a.key.toLowerCase()==='master'?-1 : b.key.toLowerCase()==='master'?1 : 0);
    if (periods.length>0) {
      try{ fs.writeFileSync(PERIODS_PATH, JSON.stringify(periods,null,2)); }catch{}
    }
    return periods;
  } catch { return []; }
}
...
function excelPath(period) { const k = (period||'master').replace(/\s+/g,'-').toUpperCase(); return path.join(DATA_DIR, `${k}.xlsx`); }
function editsPath(period) { const k = (period||'master').replace(/\s+/g,'-').toUpperCase(); return path.join(DATA_DIR, `edits_${k}.json`); }
function configPath(period) { const k = (period||'master').replace(/\s+/g,'-').toUpperCase(); return path.join(DATA_DIR, `config_${k}.json`); }
...
function excelExists(period) {
  try{
    if (!period) return fs.existsSync(path.join(DATA_DIR, 'MASTER.xlsx')) || fs.existsSync(path.join(DATA_DIR, 'master.xlsx'));
    const cand = [period, period.replace(/\s+/g,'-'), period.replace(/-/g,' '), period.toUpperCase(), period.replace(/\s+/g,'-').toUpperCase(), 'MASTER', 'master'];
    for (const c of cand){ if (fs.existsSync(path.join(DATA_DIR, `${c}.xlsx`))) return true; }
    return false;
  }catch{ return false; }
}
...
function loadExcelData(period) {
  try{
    let filePath=null;
    const tryNames=[period, period?period.replace(/\s+/g,'-'):null, period?period.replace(/-/g,' '):null, period?period.toUpperCase():null, period?period.replace(/\s+/g,'-').toUpperCase():null, 'MASTER','master'].filter(Boolean);
    for (const n of tryNames){ const p=path.join(DATA_DIR, `${n}.xlsx`); if(fs.existsSync(p)){ filePath=p; break; } }
  ...
  }catch(e){ console.log('loadExcel error', e.message); return null; }
}

app.post('/api/upload', upload.single('file'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Tidak ada file' });
  try {
    let rawPeriod = req.body.period ? req.body.period.replace(/[^a-zA-Z0-9\-_ ]/g, '').trim() : 'master';
    const period = rawPeriod.replace(/\s+/g, '-').toUpperCase();
    const label = req.body.label || period;
    const destPath = excelPath(period);
    fs.writeFileSync(destPath, req.file.buffer);
    // clear old edits for this period so data matches Excel
    try { const ep = editsPath(period); if (fs.existsSync(ep)) fs.unlinkSync(ep); } catch(e){}
...
app.get('/api/data', (req, res) => { let data = loadExcelData(req.query.period); if (!data) data = loadExcelData('master'); if (!data) return res.status(404).json({ error: 'Belum upload Excel' }); res.json(data.semuakategori); });
app.get('/api/kategori', (req, res) => { let data = loadExcelData(req.query.period); if (!data) data = loadExcelData('master'); if (!data) return res.status(404).json({ error: 'Belum upload Excel' }); res.json(data.kategoriList); });
...
app.delete('/api/periods/:key', (req,res)=>{
  try{
    ...
    if (norm==='MASTER') return res.status(400).json({error:'Master tidak boleh dihapus'});
    const keys=[rawKey, norm, rawKey.replace(/-/g,' '), rawKey.toUpperCase()];
    let deleted=[];
    for (const k of keys){
      const up=k.replace(/\s+/g,'-').toUpperCase();
      const paths=[path.join(DATA_DIR, `${k}.xlsx`), path.join(DATA_DIR, `${up}.xlsx`), path.join(DATA_DIR, `edits_${up}.json`), path.join(DATA_DIR, `config_${up}.json`), path.join(DATA_DIR, `edits_${k}.json`), path.join(DATA_DIR, `config_${k}.json`)];
      for (const p of paths){ if(fs.existsSync(p)){ try{fs.unlinkSync(p); deleted.push(require('path').basename(p));}catch{} } }
    }
    const periods=loadPeriods();
...
app.use((err,req,res,next)=>{ console.error('API ERROR', err); res.status(500).json({error:err.message}); });
