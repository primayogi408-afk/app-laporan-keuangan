...
async function loadPeriodsUI() {
  try {
    ...
    container.innerHTML = periods.map(p =>
      `<div style="display:flex;gap:4px;margin-bottom:4px;"><button class="period-btn ${p.key === selectedPeriod ? 'active' : ''}" style="flex:1;" data-key="${p.key}" onclick="gantiPeriod('${p.key}')">${p.label}</button><button title="Hapus ${p.label}" onclick="hapusPeriod('${p.key}')" style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.1);color:#ff8a80;border-radius:4px;padding:0 8px;cursor:pointer;font-size:12px;">✕</button></div>`
    ).join('');
  } catch {}
}

async function hapusPeriod(key) {
  if (key.toLowerCase()==='master') { if(!confirm('Yakin hapus MASTER? Ini data utama!')) return; } else { if(!confirm(`Hapus periode ${key}? File Excel dan editannya akan hilang permanen!`)) return; }
  try {
    const res = await fetch(`${API}/periods/${encodeURIComponent(key)}`, { method: 'DELETE' });
    const j = await res.json();
    if (j.error) { alert('Gagal hapus: '+j.error); return; }
    if (selectedPeriod===key) { selectedPeriod='master'; localStorage.setItem('selectedPeriod','master'); }
    // if no periods left, show upload
    const periods = j.periods || await fetch(`${API}/periods`).then(r=>r.json());
    if (periods.length===0) { document.getElementById('uploadOverlay').style.display='flex'; document.getElementById('appContainer').classList.add('hidden'); }
    else { await loadPeriodsUI(); await loadAll(); }
